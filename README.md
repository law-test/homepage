...
- `data/site.json`: 제9회 확정 일정·요강, 제8회 시상식·수상자·점수, 제1~7회 기록, 공모전 요강과 미확정 항목.
...
생성된 루트 HTML 18개와 `sitemap.xml`도 함께 커밋합니다. 기존 GitHub Pages 배포 경로와 `CNAME`을 유지합니다. `.nojekyll`은 생성된 파일을 그대로 배포하도록 합니다.
...
제9회 접수·응시 기간은 2026.09.21.–11.30.이며, 최신 확정 기준을 공지·응시 안내·FAQ와 포스터에 반영했습니다. 홈에는 제9회와 제2회 전국 자기주도 학습법 공모전을 함께 안내합니다. 접수 상태 표시는 한국 시간 시작·마감 시각에 따라 자동으로 바뀝니다. 참가비·환불·동점자 처리 등 별도로 확정되지 않은 세부 운영 기준은 임의로 추가하지 않습니다. 점수는 공개 허락된 값만 사용하며 원본 신청서·채점표·연락처는 저장소에 올리지 않습니다.
